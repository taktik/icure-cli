...
import {
	ImportResultDto, MedicationSchemeExportInfoDto, PatientDto,
	UserDto
} from 'icc-api'
...
api.usericc.getCurrentUser().then(user => userId = user.id)
...
vorpal
	...
	.action(async function(this: CommandInstance, args: Args) {
		const doc = await api.documenticc.createDocument({ id: api.cryptoicc.randomUuid(), author: (await api.usericc.getCurrentUser()).id, responsible: hcpartyId })
		await api.documenticc.setAttachment(doc.id, undefined, fs.readFileSync(args.path).buffer)
		latestImport = (await api.bekmehricc.importMedicationScheme(doc.id, undefined, undefined, 'fr',{}))[0]
